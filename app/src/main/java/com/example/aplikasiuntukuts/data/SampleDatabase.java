...
public abstract class SampleDatabase extends RoomDatabase {
    ...
    /**
       Mengisi database dengan data dummy yang ada di class CHEESES (String[] CHEESES)
     */
    private void populateInitialData() {
        if (cheese().count() == 0) {
            runInTransaction(new Runnable() {  //Membuat transaksi (pemindahan data) ke database
                @Override
                public void run() {
